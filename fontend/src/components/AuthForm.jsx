...
const Auth = () => {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      if (isLogin) {
        ...
        // Thêm thông tin vào bảng customers qua API của bạn
        ...
        await setDoc(doc(db, "users", user.uid), {
          ...
          username,
          role: "viewer",
          isDisabled: false, // Mặc định là chưa bị vô hiệu hóa
        });
      ...
      // Kiểm tra trạng thái `isDisabled`
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      if (!docSnap.exists()) {
        // Nếu tài khoản chưa tồn tại trong Firestore, thêm mới
        await setDoc(docRef, {
          email: user.email,
