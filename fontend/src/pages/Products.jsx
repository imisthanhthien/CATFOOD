...
const handlePriceRangeChange = (e) => {
  const value = e.target.id; // Lấy giá trị id của checkbox
  setPriceRange((prev) =>
    e.target.checked
      ? [...prev, value] // Nếu checkbox được check, thêm giá trị vào mảng
      : prev.filter((range) => range !== value) // Nếu bỏ check, loại bỏ giá trị khỏi mảng
  );
};
  ...
  const filterProducts = () => {
    const filtered = productsWithCategories.filter((product) => {
      const matchesPrice =
        priceRange.length === 0 // Không có khoảng giá nào được chọn, không lọc
          ? true
  ...
  <path stroke-linecap="round" stroke-linejoin="round" d="M21 16.811c0 .864-.933 1.406-1.683.977l-7.108-4.061a1.125 1.125 0 0 1 0-1.954l7.108-4.061A1.125 1.125 0 0 1 21 8.689v8.122ZM11.25 16.811c0 .864-.933 1.406-1.683.977l-7.108-4.061a1.125 1.125 0 0 1 0-1.954l7.108-4.061a1.125 1.125 0 0 1 1.683.977v8.122Z" />
    ...
    {[...Array(totalPages)].map((_, index) => (
      <button
        ...
        className={`px-4 py-2 text-sm rounded-3xl font-medium transition-all duration-300 transform ${
          currentPage === index + 1
            ? 'bg-blue-500 text-white shadow-lg scale-105'  // Màu xanh cho trang được chọn
